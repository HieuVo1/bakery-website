...
namespace eShopSolution.AdminApp.Service.Orders
{
    public class OrderService : IOrderService
    {
        ...
        {
            var response = await _client.GetAsync($"/api/orders/getAll");
        ...
        public async Task<ApiResult<OrderViewModel>> GetById(int orderId)
        {
            ...
            using (HttpContent content = response.Content)
            {
                ...
                if (data != null)
                {
                    if (response.IsSuccessStatusCode)
                    {
                        return JsonConvert.DeserializeObject<ApiResultSuccess<OrderViewModel>>(data);
                    }
                    return JsonConvert.DeserializeObject<ApiResultErrors<OrderViewModel>>(data);
        ...
        {
            var response = await _client.GetAsync($"/api/orders/GetOrderDetails/{orderId}");
            using (HttpContent content = response.Content)
            {
                //convert data content to string using await
                var data = await content.ReadAsStringAsync();
        ...
        {
            var response = await _client.GetAsync($"/api/orders/updateStatus/{orderId}/{status}");
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiResultSuccess<string>>(await response.Content.ReadAsStringAsync());
            }
